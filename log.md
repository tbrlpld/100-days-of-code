...
### Thoughts
I have used Flask before, but the `.flaskenv` file for the `python-dotenv` package to set the environment variable automatically is a nice addition to simplify the setup (and distribution I guess).
...
### Today's Progress
No planned coding activity in #100DaysOfWeb course today, only video.

To still code a bit, I continued with the Flask tutorial "Keep Developing" section by implementing an RSS Feed for new posts.
This should be interesting, because I never worked with RSS before.
...
Created a registration form for responsive 1998 Google landing page for #100DaysOfWeb.
...
Implemented a cars API with read all, read one and create new car endpoints.
...
Better than the class definition used by `unittest`.
...
### Today's Progress
Continued with the users API with API star for #100DaysOfWeb @TalkPython @Pybites
...
Struggling to find a good solution for a many-to-many widget.
...
My issues with it are that it does not validate the tags and also does not enforce the uniqueness.
These are typical Django issues when using the `.save()` or `.get_or_create()` method of a model.
...
### Today's Progress
Recreated the talkpython.fm header from a list using CSS #100DaysOfWeb @TalkPython @Pybites

The vertical alignment of the image was especially tricky.
You need to define an pseudo-element before it like this:
